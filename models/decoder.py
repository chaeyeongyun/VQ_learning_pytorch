...
'''
resnet50의 layer 4에서 output은 원본 h, w의 1/32이기 떄문에 vq-vae paper에서 128x128 -> 32x32로 복원했기에
이를 따라가려면 1/4크기로 output되게 해야한다. 따라서 3번 upsample 해줘야 한다.
'''
...
class Decoder_x3(nn.Module):
    def __init__(self, in_channels, num_hiddens=128, num_residual_layers=2, num_residual_hiddens=32):
        super(Decoder_x3, self).__init__()
